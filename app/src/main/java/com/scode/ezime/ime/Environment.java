...
public class Environment {
    ...
    public void onConfigurationChanged(Configuration newConfig, Context context) {
        this.onConfigurationChanged(newConfig, context, false);
    }

    public void onConfigurationChanged(Configuration newConfig, Context context, boolean isForce) {
        if (mConfig.orientation != newConfig.orientation || isForce) {
            WindowManager wm = (WindowManager) context
                    .getSystemService(Context.WINDOW_SERVICE);
            Display d = wm.getDefaultDisplay();
            mSKBWidth = (int) (d.getWidth() * Settings.getSizeRatioPercent());
            mSKBHeight = (int) (d.getHeight() * Settings.getSizeRatioPercent());
            mScreenWidth = d.getWidth();
            mScreenHeight = d.getHeight();
            int scale;
            if (mSKBHeight > mSKBWidth) {
                mKeyHeight = (int) (mSKBHeight * KEY_HEIGHT_RATIO_PORTRAIT);
                mCandidatesAreaHeight = (int) (mSKBHeight * CANDIDATES_AREA_HEIGHT_RATIO_PORTRAIT);
                scale = mSKBWidth;
            } else {
                mKeyHeight = (int) (mSKBHeight * KEY_HEIGHT_RATIO_LANDSCAPE);
                mCandidatesAreaHeight = (int) (mSKBHeight * CANDIDATES_AREA_HEIGHT_RATIO_LANDSCAPE);
                scale = mSKBHeight;
            }
