...
import com.scode.ezime.IPinyinDecoderService;
import com.scode.ezime.R;
import com.scode.ezime.utils.Loger;
import com.scode.ezime.view.DragView;
...
public class PinyinIME extends InputMethodService {
    ...
    private int mIMEHeight, mIMEWidth;//键盘宽高

    private static PinyinIME ime;
    ...
    public void onCreate() {
        ...
        mEnvironment.onConfigurationChanged(getResources().getConfiguration(),
                this);
        mDialog = this.getWindow();//InputMethodService返回的一个输入法窗口对象，也是Dialog
        mRootView = mDialog.getWindow().getDecorView();
    ...
    private void updateComposingText(boolean visible) {
        if (!visible) {
            mCandidatesContainer.setBackground(getApplicationContext().getResources().getDrawable(R.color.bg_transparent));
            mComposingView.setVisibility(View.INVISIBLE);
        } else {
            mCandidatesContainer.setBackground(getApplicationContext().getResources().getDrawable(R.color.bg_white_4));
    ...
    private void showCandidateWindow(boolean showComposingView) {
        if (mEnvironment.needDebug()) {
            Loger.i("Candidates window is shown. Parent = "
    ...
    public View onCreateInputView() {
        ...
        //todo
        mSkbContainer.setOnEventListener(new DragView.OnEventListener() {
            @Override
            public void onTouchMove(float x, float y) {
                if (mRootView != null) {
                    WindowManager.LayoutParams params =
                            (android.view.WindowManager.LayoutParams) mRootView.getLayoutParams();
                    Loger.i("OnMove before x>" + params.x + " y>" + params.y);

                    params.x += x;
                    params.y += y;

                    int moveW = (mEnvironment.getmScreenWidth() - mEnvironment.getSKBWidth()) / 2;

                    if (params.x < -1 * moveW) {
                        params.x = -1 * moveW;
                    } else if (params.x > moveW) {
                        params.x = moveW;
                    }

                    if (!Settings.isKbShow()) {
                        params.x = mEnvironment.getSKBWidth() + mEnvironment.getmScreenWidth() / 2;
                    }
                    int moveH = (mEnvironment.getmScreenHeight() - mEnvironment.getSkbHeight()) / 2 + 50;

                    if (params.y < -1 * moveH) {
                        params.y = -1 * moveH;
                    } else if (params.y > moveH) {
                        params.y = moveH;
                    }
                    Settings.setAnchorX(params.x);
                    Settings.setAnchorY(params.y);
    ...
    public void onWindowShown() {
        super.onWindowShown();
        //todo
        //params.width = (int) (this.getResources().getDisplayMetrics().widthPixels * Settings.getSizeRatioPercent()); //设定好输入窗体的宽度，是通过屏蔽宽度的1/2
        //+ DpUtil.dip2px(getApplicationContext(), Settings.mDragBarH)
        //params.height = Environment.getInstance().getSkbHeight() + DpUtil.dip2px(getApplicationContext(), Settings.mDragBarH);
        WindowSetting();
    }

    public void WindowSetting() {
        if (mDialog != null) {
            Log.d("carlIME", "dialog != null");
            WindowManager.LayoutParams params = new WindowManager.LayoutParams();
            params.gravity = Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL;
            params.x = Settings.getAnchorX(this.getResources().getDisplayMetrics().widthPixels);
            params.y = Settings.getAnchorY(this.getResources().getDisplayMetrics().heightPixels);
...
//            mDialog.getWindow().setBackgroundDrawableResource(R.color.cardview_light_background);
