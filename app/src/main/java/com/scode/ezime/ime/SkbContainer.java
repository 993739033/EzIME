...
import com.scode.ezime.utils.DpUtil;
...
public class SkbContainer extends RelativeLayout implements OnTouchListener {
    ...
    private DragView.OnEventListener mOnEventListener;


    public void setOnEventListener(DragView.OnEventListener l) {
        mOnEventListener = l;
        if (mDragView != null)
...
//        }
    ...
    public void updateSkbLayout() {
        //todo 这里统一缩放键盘
        float ratio = Settings.getSizeRatioPercent();
        int screenWidth = (int) (mEnvironment.getSKBWidth());
        int keyHeight = (int) (mEnvironment.getKeyHeight());
        int skbHeight = (int) (mEnvironment.getSkbHeight());
        ...
        if (null == mDragView) {
            mDragView = (DragView) findViewById(R.id.view_drag);
            ViewGroup.LayoutParams lp = (LayoutParams) mDragView.getLayoutParams();
            lp.width = (int) (screenWidth / 1.6f);
        ...
        //todo 这里加上隐藏操作
        View layout_hide = findViewById(R.id.layout_hide);
        layout_kb = findViewById(R.id.layout_kb);
        ImageView btn_hide = findViewById(R.id.btn_hide);
        if (layout_hide != null) {
            layout_hide.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (Settings.isKbShow()) {
                        Settings.setKbShow(false);
                        layout_kb.setVisibility(GONE);
                        btn_hide.setBackgroundResource(R.drawable.icon_arrow_show);
                        postInvalidate();
                        //右划到最右展示
                        mOnEventListener.onTouchMove(1920, 0);
                    } else {
                        Settings.setKbShow(true);
                        layout_kb.setVisibility(VISIBLE);
                        btn_hide.setBackgroundResource(R.drawable.icon_arrow_hide);
                        postInvalidate();
                        mOnEventListener.onTouchMove(0, 0);
                    }
                }
            });
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        Loger.i("SkbContainer onMeasure");
        Environment env = Environment.getInstance();
        int measuredHeight = DpUtil.dip2px(getContext(), Settings.mDragBarH);
        measuredHeight += env.getSkbHeight();
        int w = MeasureSpec.getSize(widthMeasureSpec);
        int h = measuredHeight;
        Loger.i("SkbContainer W" + w + " H" + h);
        if (!Settings.isKbShow()) {
            w = DpUtil.dip2px(mContext, 30);
            h = DpUtil.dip2px(mContext, 100);
        }
        int wspec = MeasureSpec.makeMeasureSpec(w,
                MeasureSpec.AT_MOST);
        int hspec = MeasureSpec.makeMeasureSpec(h,
                MeasureSpec.AT_MOST);
